...
| Job | Command | Log |
| --- | --- | --- |
| from-scratch rebuild - DONE 20:18, rc 0 at every stage | `build-system.sh FRESH=1`: stamps archived to `.stamps/legacy/reset-20260913T142648`, previous trees set aside as `work/sysroot.old-20260913T142648` and `work/build.old-20260913T142648` (kept, not deleted; ~19 GB to reclaim); stage 01 and 02 as `build`, stage 04 and `make test-libc-unwind` as root, from snapshot eb72a56 | `/root/kryptik/logs/build-system.out`, `toolchain.log`, `temp-tools.log`, `system.log`, `libc-unwind.log` |
| post-build follow-on - RUNNING since 20:19 on snapshot d20f237 | `post-build.sh`: musl binaries rebuilt (kryptikd cb36a166, wlproxy db341c97), incremental `make system` (the dwl, desktop, kryptikd and tests steps and everything after them), then `make kernel`, `make media` for release A (`0.1.20260913.<sha8>`) and release B (`<A>.1`) | `/root/kryptik/logs/post-build.out`, `system2.log`, `kernel.log`, `media-a.log`, `media-b.log` |
