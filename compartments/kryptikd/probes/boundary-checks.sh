...
# These fail when a filter is tightened so far that ordinary programs break.
...
# A failed redirection is status 1 in bash but 2 in dash, so normalise it.
...
# ls opens one descriptor of its own to read the directory: four entries.
...
dies_with() {
    local sig="$1" p
    # argv[0] is set with python's execvp: dash has no `exec -a`.
